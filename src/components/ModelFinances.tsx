...
const ModelFinances = ({ modelId, modelName, currentUserEmail, onBack }: ModelFinancesProps) => {
  ...
  const totalIncome = onlineData.reduce((sum, d) => {
    const dailyIncome = ((d.cbIncome + d.spIncome + d.sodaIncome) * 0.05 + d.cam4Income + d.transfers) * 0.3;
  ...
  const platformSummary = [
    { platform: 'Chaturbate', tokens: totalCbIncomeTokens, income: totalCbIncomeTokens * 0.05 * 0.3 },
    { platform: 'Stripchat', tokens: totalSpIncomeTokens, income: totalSpIncomeTokens * 0.05 * 0.3 },
    { platform: 'CamSoda', tokens: totalSodaIncomeTokens, income: totalSodaIncomeTokens * 0.05 * 0.3 },
    { platform: 'Cam4', tokens: totalCam4, income: totalCam4 * 0.3 },
  ];
      ...
      <Card className="p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={onlineData.map(d => ({ 
            date: formatDate(d.date), 
            CB: d.cbIncome * 0.05 * 0.3, 
            SP: d.spIncome * 0.05 * 0.3, 
            Soda: d.sodaIncome * 0.05 * 0.3,
            Cam4: d.cam4 * 0.3,
            Transfers: d.transfers * 0.3
          }))}>
            <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
            <XAxis dataKey="date" className="text-xs" />
            <YAxis className="text-xs" />
